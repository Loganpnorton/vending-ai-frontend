-- Fix Polling Function for Actual Machines Table Structure
-- Based on the actual columns: machine_code, machine_token, is_paired
...
AS $$
BEGIN
    -- Look for a machine that was paired using this pairing code
    -- We'll check if the pairing code exists and if a machine has been marked as paired
    SELECT 
        m.machine_code,  -- Using machine_code instead of machine_id
        m.machine_token
    INTO machine_id, machine_token
    FROM pending_machine_links pml
    JOIN machines m ON m.machine_code = pml.machine_code  -- Join on machine_code
...
BEGIN
    ...
    SET status = 'completed',
        used_at = NOW(),
        machine_code = machine_code_param  -- Store the machine_code reference
    WHERE pairing_code = pairing_code_param;
